...
def build_composite_label_zpl(batch_items):
    ...
    for idx, item in enumerate(batch_items):
        ...
        if raw_bc == "EMPTY_ROW":
            zpl.append(f"^FO340,{current_y + 35}^A0N,28,24^FD# {idx+1}: EMPTY ROW^FS")
        else:
            # Column A: 2D Data Matrix
            zpl.append(f"^FO100,{current_y}^BXN,8,200,,,,^FS^FH\\^FD{raw_bc}^FS")
            
            # Column B: Two-line text layout with ^FB text block command enabled
            zpl.append(f"^FO340,{current_y + 15}^A0N,30,26^FB1500,2,0,L^FD#{idx+1}: {hyphen_txt}^FS")
...
if __name__ == "__main__":
    # Setup test folder environment locally
